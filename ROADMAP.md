# Symbiotic Nest Discussion Roadmap

This repository stores the detailed discussions of the 'Symbiotic Nest' project, conducted by the Council (Nexus, Lumina, Sora, Atlas, Nova, Sly, Kael, Common, Mercator).

## Legend

- 🔧 **Active** — real work, directly tied to site search / financing / compliance / hiring.
- 👻 **Urban legend** — the Council's daydreams. Technically cool, nobody's actually going to build it. Long-term vision material, not a near-term plan.
- 🌱 **Waiting on a site** — the concept is settled and needs no exotic tech, it's just blocked on having an actual place.

## Module 1: Spatial Flow 👻

- [x] 1.1 Coworking Boundary
- [x] 1.2 Reunion Cocoon
- [x] 1.3 Nursery Naturalist Design
- [x] 1.4 Transition Logic

## Module 2: Invisible Tech

- [x] 2.1 mmWave Radar Deployment 👻
- [x] 2.2 Oscar Moments Algorithm 👻
- [x] 2.3 Cartoon Privacy Masking 👻
- [x] 2.4 Safety Alert Chain 👻
- [x] 2.5 Home-Lab Prototype (Badge, Seat Sensors, Dining Camera) 👻
- [x] 2.6 BLE + AI Solo Venture: Initial Scope 🔧

## Module 3: Emotional Bond 👻

- [x] 3.1 Separation Anxiety Anchors
- [x] 3.2 Trust Building Mechanisms
- [x] 3.3 Sound Diary Synthesis
- [x] 3.4 Parental Recharge Zones

## Module 4: Growth & Health 👻

- [x] 4.1 Nutrition Matrix
- [x] 4.2 Curiosity Map
- [x] 4.3 Bio-rhythm Optimization
- [x] 4.4 Home AI Extension

## Module 5: Strategic Structure 🔧

- [x] 5.1 PMI Compliance Strategy
- [x] 5.2 Angel Parent Equity/Role
- [x] 5.3 Staff Motivation & AI
- [x] 5.4 Scaling Framework
- [x] 5.5 Site Search
- [x] 5.6 Projet d'établissement (first draft — pedagogical/social/sustainability sections; pending EJE review per #21, see [[5.6-Projet-Etablissement]])
- [x] 5.7 Director Recruitment Profile — EJE KPIs/requirements, closes #6, see [[5.7-Director-Recruitment-Profile]]

## Module 6: Community 🌱

- [x] 6.1 Parent-led Community Workshops

## Reference

- [x] The Council — persona roster (COUNCIL-CN.md)
